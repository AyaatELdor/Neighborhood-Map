...
function initMap() {
      ...
      var mapOptions = {
          center: new google.maps.LatLng(30.060668, 31.219883), 
          // the initial location of cairo on map lat and lng
          zoom: 12,
          // the initial zoom to display area 
          mapTypeId: google.maps.MapTypeId.ROADMAP
          // type of map ROADMAP (normal, default 2D map)
      };
      // set default values to map that will load in map-canvas
...
var Model = [{
          "name": "Burger Factory", 
          "latlng": [30.059875598754438, 31.221953590277508] 
      },
      {
          "name": "Left Bank", //the name of place
          "latlng": [30.073034918695033, 31.221981542482556]
      },
      {
          "name": "Pottery Cafe", //the name of place
          "latlng": [30.064569817454906, 31.215708902077974] 
      },
      {
          "name": "The Coffee Bean & Tea Leaf",
          "latlng": [30.064377087896453, 31.215763092041016]
      },
      {
          "name": "Diwan Bookstore",
          "latlng": [30.05925140815715, 31.224002838134766]
      }
];
...
var appViewModel = function() {
      var ctrl = this; 
      // use this variable to define this scope only 
      ctrl.markers = ko.observableArray([]);
      // defined that as array using observable
      ctrl.allLocations = ko.observableArray([]);
      // defined that as array using  observabl


      ctrl.filter = ko.observable(''); 
      // defined that as sting using  observabl
      ctrl.search = ko.observable('');
      // defined that as sting using  observabl
      ...
      ctrl.filteredArray = ko.computed(function() {
          return ko.utils.arrayFilter(ctrl.allLocations(), function(item) {
          // loop on all in allLocations using each item 
              if (item.name.toLowerCase().indexOf(ctrl.filter().toLowerCase()) !== -1) { 
              // parse name in item to lower case and check it if contain same user words enter 
                  if (item.marker)
                      item.marker.setMap(map); 
                      //if compare return result call marker and show it only on map
              } else {
                  if (item.marker)
       ...
       Model.forEach(function(place) {
            ...
            $.getJSON(foursquareUrl,function(data){
                ...
                //check if return data
                    ...
                    // place info will return on venues[0]
                    allLocations.push(item); 
                    //push places in allLocations array
                    ...
                    locDataArr.push(location); 
                    //set all info in locDataArr array
                    placeMarkers(allLocations, place, location, map, markers); 
                    // call placeMarkers to display points on map
                }
            }).fail(function(data){
        ...
        function placeMarkers(allLocations, place, data, map, markers) {
              ...
              //set lat and lng in google.maps object
              var marker = new google.maps.Marker({ 
              // set value of point 
                  position: latlng, 
                  // position of point 
                  map: map, 
                  // to overwrite on last points on map
                  animation: google.maps.Animation.DROP, 
                  // set point animation 
                  content: data.name + "<br>" + data.loc 
                  // print name and location on tip toggal`
              });
              ...
              markers.push(marker); 
              // push marker info in markers array 
              allLocations()[allLocations().length - 1].marker = marker; 
              // set last index value in allLocations that have marker equal marker
              ...
              google.maps.event.addListener(marker, 'click', function() {
                  toggleBounce(marker); 
                  //call toggleBounce using marker data
              });
        }
